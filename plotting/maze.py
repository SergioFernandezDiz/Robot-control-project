...
def plot_episode_metrics(time_log,state_log,base_pos_log,ee_pos_log,dist_to_bulb_log,dist_to_fixture_log,base_vel_log,arm_vel_norm_log,filename,start_pos, bulb_pos_xy, wall_stop_pos, bulb_pos_3d, fixture_pos_3d,TaskState, robot_name: str):
    ...
    # First plot: Task State over Time
    state_colors = {
        TaskState.NAVIGATE_TO_BULB.value: 'lightblue',
        TaskState.APPROACH_BULB.value: 'deepskyblue',
        TaskState.NAVIGATE_IN_MAZE.value: 'lightgreen',
        TaskState.TRANSPORT_TO_WALL.value: 'orange',
        TaskState.APPROACH_WALL.value: 'darkorange',
        TaskState.RETURN_TO_START.value: 'purple'
    }
    ...
    # Second - plot 2D for base trajectory
    state_to_segment = {
        TaskState.NAVIGATE_TO_BULB.value: 1,       
        TaskState.NAVIGATE_IN_MAZE.value: 1,       
        TaskState.TRANSPORT_TO_WALL.value: 2,      
        TaskState.STOP_IN_FRONT_OF_WALL.value: 2,  
        TaskState.RETURN_TO_START.value: 3         
    }
    ...
    segments_info = [
        (1, 'Navigate to Pick Point'),
        (2, 'Transport to Drop/Wall Point'),
    ...
    key_points = [
        ...
        (bulb_pos_xy, 'Pick Position'),
        (wall_stop_pos, 'Drop Stop Position')
    ]
    ...
    ee_phases = [
        {'state': TaskState.APPROACH_BULB.value, 'title': 'EE Approach Pick Target', 'target': bulb_pos_3d},
        {'state': TaskState.APPROACH_WALL.value, 'title': 'EE Approach Drop Target', 'target': fixture_pos_3d},
    ]
